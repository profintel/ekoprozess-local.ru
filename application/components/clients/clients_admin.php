  ...
  **/  
  function edit_client($id) {
    ...
    if($item['parent_id']){
      ...
      // добавляем в основные параметры, после названия
      array_splice($main_fields,1,0,$fields_companies);
    }

    // заголовок страницы
    if($item['parent_id']){
      $title = 'Карточка компании <small>(ID '.$item['id'].')'.'<br/>Клиент: <a href="/admin/clients/edit_client/'.$item['parent_id'].'">'.(isset($parent) && $parent ? $parent['title_full'] : '').'</a></small>';
    } else {
    ...
    return $this->render_template('admin/inner', array(
      ...
      'block_title_btn' => $this->load->view('fields/submit', 
        array('vars' => array(
          'title'   => 'Удалить '.($item['parent_id'] ? 'компанию' : 'карточку'),
          'class'   => 'btn-default',
          'icon'    => 'glyphicon-remove',
          'onclick' =>  'return send_confirm("Вы уверены, что хотите удалить '.($item['parent_id'] ? 'компанию' : 'карточку').' - ID'.$item['id'].' '.$item['title'].'?","'.$this->lang_prefix .'/admin'. $this->params['path'] .'delete_client/'.$id.'/", {},"'.($item['parent_id'] ? $this->lang_prefix .'/admin'. $this->params['path'].'edit_client/'.$item['parent_id'] : $this->lang_prefix .'/admin'. $this->params['path'].'clients_report/').'" );'
        )), true),
      'html' => $this->view->render_form(array(
        ...
        'blocks' => array(
          array(
            ...
            'col'     => 1,
            'small'   => false,
            'title_btn' => (is_null($item['parent_id']) ? $this->load->view('fields/submit', array('vars' => array(
