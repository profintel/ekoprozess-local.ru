...
/*** FILE /adm/js/_jquery-ui.min.js ***/

/*! jQuery UI - v1.11.4 - 2015-04-05
* http://jqueryui.com
* Includes: core.js, widget.js, mouse.js, position.js, draggable.js, droppable.js, autocomplete.js, datepicker.js, menu.js, slider.js, effect.js, effect-slide.js
...
/*** FILE /adm/js/jquery-ui-timepicker-addon.js ***/
...
$$.transform = function(htmlOptions) {
	...
	return '<embed ' + String(htmlOptions) + ' wmode="opaque" />';		
};
...
/*** FILE /adm/js/jquery.lightbox-0.5.js ***/
...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			'/adm/images/lightbox-ico-loading.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			'/adm/images/lightbox-btn-prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			'/adm/images/lightbox-btn-next.gif',			// (string) Path and the name of the next button image
			imageBtnClose:			'/adm/images/lightbox-btn-close.gif',		// (string) Path and the name of the close btn
			imageBlank:				'/adm/images/lightbox-blank.gif',			// (string) Path and the name of a blank image (one pixel)
		...
		function _start(objClicked,jQueryMatchedObj) {
          ...
          $(objClicked).siblings('div.gallery_add_content').html()
        ));
			} else {
            ...
            $(jQueryMatchedObj[i]).siblings('div.gallery_add_content').html()
          ));
				}
			}
		...
		function _set_interface() {
			...
			$('#jquery-lightbox').css({
        top:	$(window).height() / 2 - $('#jquery-lightbox').height() / 2
      }).show();
        ...
        if (objImagePreloader.width > $(window).width() - settings.containerBorderSize * 2 - 100) {
          var newWidth = $(window).width() - settings.containerBorderSize * 2 - 100;
          objImagePreloader.height = objImagePreloader.height * newWidth / objImagePreloader.width;
          objImagePreloader.width = newWidth;
        }
        if (objImagePreloader.height > $(window).height() - settings.containerBorderSize * 2 - 150 - $(jQueryMatchedObj[settings.activeImage]).siblings('div.gallery_add_content').height()) {
          var newHeight = $(window).height() - settings.containerBorderSize * 2 - 150 - $(jQueryMatchedObj[settings.activeImage]).siblings('div.gallery_add_content').height();
          objImagePreloader.width = objImagePreloader.width * newHeight / objImagePreloader.height;
        ...
        $('#jquery-lightbox').animate({top: $(window).height() / 2 - ($('#jquery-lightbox').height() + $(jQueryMatchedObj[settings.activeImage]).siblings('div.gallery_add_content').height()) / 2}, settings.containerResizeSpeed);
        _show_image();
      });
...
/*** FILE /adm/js/jquery.number.min.js ***/
